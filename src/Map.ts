...
import { Log as cgilLog, levelLog } from 'cgil-log';
// eslint-disable-next-line import/extensions,import/no-unresolved
import {
  MODULE_NAME, MODULE_VERSION, BUILD_DATE,
// eslint-disable-next-line import/extensions,import/no-unresolved
} from './config';

let isItDEV = false;
if (typeof process !== 'undefined') {
  isItDEV = process.env.NODE_ENV === 'development';
}
export const DEV = isItDEV;
// eslint-disable-next-line no-undef
const getLog = (ModuleName:string, verbosityDev:levelLog, verbosityProd:levelLog):cgilLog => (
  // eslint-disable-next-line new-cap
  (DEV) ? new cgilLog(ModuleName, verbosityDev) : new cgilLog(ModuleName, verbosityProd)
);

export const version = MODULE_VERSION;
export const name = MODULE_NAME;
export const build = BUILD_DATE;
const log = getLog(MODULE_NAME, 4, 4);
...
function getWmtsSource(WMTSCapabilitiesParsed:any, layerName:string) {
